...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newuser._spec = request.POST['spec']
            newuser.save()
            messages.success(request, f'Your account has been created! You can now login!')
            return redirect('login')
    else:
        form = UserRegisterForm()
    return render(request, 'users/register_.html', {'form':form})
...
def login_user(request):
    if request.method == 'POST':
        ...
        # username = request.POST['username']
        get_object_or_404(User, email=request.POST['username'])
        username = get_object_or_404(User, email=request.POST['username'])
...
def my_profile(request):
    ...
    user_events = Event.objects.filter(user=you)
    user_event_requests = EventRequest.objects.filter(to_user=you)
    friends = p.friends.all()
    ...
    context = {
        ...
        'post_count': user_posts.count,
        'events': user_events,
        'events_req': user_event_requests,
    }
